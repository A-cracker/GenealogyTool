    ...
    _createClass(TreeBuilder, [{
      ...
      value: function _update(source) {
        ...
        nodes.append('foreignObject').filter(function (d) {
          ...
          const event = d3.event.changedTouches[0];
          mobileLongPress = setTimeout(function () {
            if (d.data.isMarriage) {
              opts.callbacks.marriageRightClick.call(this, d.data.extra, d.data.id, event);
            } else {
              opts.callbacks.nodeRightClick.call(this, d.data.name, d.data.extra, d.data.id, event);
            }
          },400)
        }).on('touchend', function(){
          clearTimeout(mobileLongPress)
          ...
          if (d.data.isMarriage) {
            opts.callbacks.marriageRightClick.call(this, d.data.extra, d.data.id, d3.event);
          } else {
            opts.callbacks.nodeRightClick.call(this, d.data.name, d.data.extra, d.data.id, d3.event);
          }
        });
      }
    }, {
      ...
      value: function _siblingLine(d, i) {
        ...
        // Not first marriage
        if (d.number > 0) {
          ny -= Math.round(nodeHeight * 6.5 / 10);
        }
    ...
    init: function init(data) {
      ...
      return {
        ...
        zoomToNode: function zoomToNode(nodeId) {
          ...
          if (node) {
            _zoomTo(node.x, node.y + 300, zoom, duration);
          }
        },
