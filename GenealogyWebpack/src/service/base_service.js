...
export default class BaseService {
	...
	constructor() {
		...
		this.CACHE_RULES = {
			...
			PARENT_CHILDREN: {
				identifyKeys: ['parentObjType', 'subObjType', 'parentObjId','subObjIdsCacheKey', '|removeObjId|removeObjIds|removeSubObjId|removeSubObjIds'],
				get: (params, placeholders, args) => {
					let parentObj = this._getObjInCache(placeholders['parentObjId'], placeholders['parentObjType'], params, placeholders);
					if(! parentObj) {
						parentObj = this._generateUnfetchObjAndCache(placeholders['parentObjId'], placeholders['parentObjType'], params, placeholders);
	...
	queryFromCache(params, placeholders, callbacks) {
		...
		if(cacheSearches) {
			cacheable.objs = _sortingCache(params, placeholders, 
	...
	async _handleDetailsFetch(params, callbacks, placeholders, json) {
		...
		for(let copyKey of Config.SERIES_COPY_PARM_NAMES) {
			if(! params.hasOwnProperty(copyKey)
				|| ! params[copyKey]) {
				continue;
			}
			queryParams[copyKey] = params[copyKey];
		}
		return await this.queryTemplate(queryParams, {
			onSuccess: (objs) => {
