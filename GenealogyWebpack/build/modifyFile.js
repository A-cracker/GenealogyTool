...
function modifyMainPage() {
    ...
    fs.readFile(mainPageName, (err, data) => {
        ...
        newContent = newContent.replace(/resource\/js/g, "${resourceUrl}/js");
...
function modifyJs() {
    ...
    const  files = fs.readdirSync(_assetsPath + '/../../WebContent/resource/js/')

    files.forEach( f=> {
        fs.copyFileSync(_assetsPath + '\\..\\..\\WebContent\\resource\\js\\' + f, _assetsPath + '/js/' + f)
    })
}
